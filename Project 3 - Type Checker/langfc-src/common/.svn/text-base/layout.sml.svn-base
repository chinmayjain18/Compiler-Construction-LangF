...
fun seq ts =
   let
      val len = fold (ts, 0, fn (t,n) => n + length t)
   in
      case len of
         0 => empty
...
fun indent (t, n) =
   T {length = length t, tree = Indent (t, n)}
...
fun outputWidth (outStrm, width, t) =
   print {tree = t,
          lineWidth = width,
          print = fn s => TextIO.output (outStrm, s)}
fun makeOutputWidth (width, layoutX) (outStrm, x) =
   outputWidth (outStrm, width, layoutX x)

val defaultWidth: int = 128
fun output (outStrm, t) = outputWidth (outStrm, defaultWidth, t)
fun makeOutput layoutX (outStrm, x) = output (outStrm, layoutX x)
...
val array = vector o Array.vector
...
fun tuple4 (l1, l2, l3, l4) (x1, x2, x3, x4) =
