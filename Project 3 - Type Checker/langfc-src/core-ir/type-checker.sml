      ...
      struct
         ...
         val initial =
            ...
            (fn ((tycon, arity, dacons), env) =>
             extend (env, singletonTyCon (tycon, {arity = arity, dacons = dacons})))
            initial
            [(C.TyCon.array, 1, []),
                     ...
                     case Env.lookupDaCon (env, dacon) of
                       ...
                       | SOME {tyvars = formal_tyvars,
                           ...
                           let
                              ...
                              fun substAll ty =
                                 ListPair.foldl
                                 (fn (actual_tyarg, formal_tyvar, ty) =>
                                  C.Type.subst (ty, (actual_tyarg, formal_tyvar)))
                                 ty
                                 (actual_tyargs, formal_tyvars)
                              val formal_arg_tys =
                                 C.Type.substsL (formal_arg_tys,
                                                 ListPair.zip (actual_tyargs,
                                                               formal_tyvars))
