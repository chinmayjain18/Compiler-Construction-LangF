...
for f in test*.lgf; do
    ...
    rm -f $t.log ;
    echo "----------" >> $t.log ;
    echo "\$ ../../bin/langfc -Cverbose=false -Cerr-strm-report-file=$t.scan.err -Ckeep-scan=true -Cstop-scan=true $t.lgf" >> $t.log ;
    rm -f $t.lck ; touch $t.lck ;
    ( ../../bin/langfc -Cverbose=false -Cerr-strm-report-file=$t.scan.err -Ckeep-scan=true -Cstop-scan=true $t.lgf 1> $t.stdout 2> $t.stderr ; rm $t.lck ) &
    k=0 ;
...
echo
echo
tokTot=$((toksPass + toksFail))
errTot=$((errPass + errFail))
echo "scanner (toks): "$((ptPass))" / "$((ptTot))"  ("$(( (100 * toksPass) / tokTot ))"%)"
if [ ! -z "${toksFails}" ]; then
...
echo
echo
echo "scanner (tot): "$((toksPass + errPass))" / "$((tokTot + errTot))"  ("$(( (100 * (toksPass + errPass)) / (tokTot + errTot) ))"%)"
